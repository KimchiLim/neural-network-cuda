	...
	unsigned int *count)
{
	...
	int i;
	char tmp[4];
    int GottaImpressTheCompiler = 0;
	GottaImpressTheCompiler++;
	...
	GottaImpressTheCompiler = fread(tmp, 1, 4, ifp);
	if (mnist_bin_to_int(tmp) != 2051) {
		return_code = -2; /* Not a valid image file */
		goto cleanup;
	}

	GottaImpressTheCompiler = fread(tmp, 1, 4, lfp);
	...
	*data = (mnist_data *)malloc(sizeof(mnist_data) * image_cnt);
	...
	for (i = 0; i < image_cnt; ++i) {
		...
		GottaImpressTheCompiler = fread(read_data, 1, 28*28, ifp);
		...
		GottaImpressTheCompiler = fread(tmp, 1, 1, lfp);
		d->label = tmp[0];
	}
